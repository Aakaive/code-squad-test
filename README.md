...
## 2단계 : Solution : 1단계에 새로운 요소를 더한 솔루션
### 1. settingPhase : 변수 선언 및 카드 리스트와 플레이어 객체 생성
- shuffleCard() 메소드를 이용하여 랜덤으로 섞인 18장의 카드 리스트 생성
- Player 객체의 인스턴스 p1, p2 생성으로 플레이어1과 플레이어2에 대한 정보(이름)를 입력받고 게임을 시작한다.
- Player 객체의 속성은 name, score, BonusScore로 이루어져 있으며, 각각 이름과 점수, 카드를 맞출 경우 획득 가능한 점수를 의미한다.
- 각 플레이어의 차례를 의미하는 enum PlayerTurn {P1, P2}을 선언함으로써, PlayerTurn은 플레이어1과 플레이어2의 속성만을 지닐 수 있음을 명시한다.
- PlayerTurn의 인스턴스인 turn을 생성과 동시에 P1으로 초기화함으로써 turn이 플레이어1부터 시작되도록 설정한다.
### 2. standbyPhase : 게임 테이블에 놓여진 카드를 표현하고, 선택할 카드를 입력 받는다.
- Player 인스턴스 p1과 p2의 score를 출력하여 점수를 확인한다.
- PlayerTurn의 인스턴스인 turn이 P1이므로, Player의 인스턴스 중 p1부터 카드를 선택할 수 있다.
- 선택할 카드의 좌표(x, y)를 두 번 입력받는다.
### 3. openPhase : 선택한 카드를 뒤집어 본다.
- openCard() 메소드를 이용하여 선택한 카드의 종류를 표시한 카드 리스트를 출력한다.
- 선택한 두 카드의 종류가 일치할 경우, Player 클래스 내부에 선언된 getScore 메소드를 활용하여 Player 인스턴스의 score에 BonusScore를 가산한다.
- score에 BonusScore를 가산한 후, BonusScore를 두배로 증가시킨다.(룰)
- 만약, 선택한 두 카드가 일치하지 않을 경우 score의 가산없이 BonusScore를 초기값인 10으로 초기화한다.
- 또한, turn의 속성을 변경한다.(P1일 경우 P2로, P2일 경우 P1으로.)
### 4. endPhase : 게임종료 조건을 확인하고, 조건을 충족할 경우 게임을 종료한다.
- 카드 리스트 내부의 중복요소를 파악하고, 0으로 이루어진 요소를 파악함으로써 카드 리스트 내부에 남아 있는 카드로 하여금 게임 속행 여부를 판단할 수 있다.
- 게임종료 조건이 충족될 경우 p1과 p2의 점수를 출력하고 승자를 축하하는 메시지를 출력한다.
- 또한, break문을 사용하여 루프를 빠져나오고 프로그램을 종료한다.
### 5. loop : 게임종료 조건이 충족되지 않을 경우 standbyPhase로 돌아가며 반복한다.